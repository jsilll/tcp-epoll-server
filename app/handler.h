...
public:
     ...
     */
    [[nodiscard]] static bool OnNew(const sockaddr_in &addr, std::vector<std::byte> &out_buf) noexcept {
     ...
     */
    [[nodiscard]] static bool OnRead(const sockaddr_in &addr,
        ...
        std::cout << "Received '" << std::string(reinterpret_cast<const char *>(in_buf.data()), len) << "' from "
    ...
    */
    static void OnError(const sockaddr_in &addr, const tcp::Error &error) noexcept {
        std::cout << "Error from " << inet_ntoa(addr.sin_addr) << ":" << ntohs(addr.sin_port) << ": " << error.what()
                  << std::endl;
    }
};